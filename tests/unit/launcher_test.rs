...
mod tests {
    use crate::test_utils::{
        McpFixtures, ProfileBuilder, WorkspaceVerifier, create_temp_storage,
    ...
    fn test_extension_installation() {
        ...
        assert!(WorkspaceVerifier::verify_extension_installed(&profile_workspace, &ext.id).is_ok());

        // Verify context file
        assert!(
            WorkspaceVerifier::verify_context_file(&profile_workspace, &ext.id, "GEMINI.md")
                .is_ok()
        );
    }
    ...
    fn test_context_file_creation() {
        ...
        let content1 =
            WorkspaceVerifier::verify_context_file(&profile_workspace, "test1", "GEMINI.md")
                .unwrap();
        assert!(content1.contains("Echo Test Extension"));

        let content2 =
            WorkspaceVerifier::verify_context_file(&profile_workspace, "test2", "GEMINI.md")
                .unwrap();
        assert!(content2.contains("Context Only Extension"));

        let content3 =
            WorkspaceVerifier::verify_context_file(&profile_workspace, "test3", "GEMINI.md")
                .unwrap();
    ...
    fn test_environment_preparation() {
        ...
        profile
            .environment_variables
            .insert("TEST_OVERRIDE".to_string(), "overridden".to_string());
        profile
            .environment_variables
            .insert("EXPANDED_VAR".to_string(), "$TEST_BASE_VAR".to_string());
    ...
    fn test_working_directory_expansion() {
        let (_launcher, _, _) = create_test_launcher();
        ...
        for (input, expected_prefix) in test_cases {
            let profile = ProfileBuilder::new("dir-test").build();
            let mut p = profile;
    ...
    fn test_extension_validation() {
        ...
        // Test invalid extension - no command
