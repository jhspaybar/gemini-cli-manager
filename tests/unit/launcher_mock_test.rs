...
mod tests {
    ...
    use crate::test_utils::{ProfileBuilder, McpFixtures, create_temp_storage};
    ...
    fn test_launch_with_profile_setup() {
        ...
        let mut profile = ProfileBuilder::new("full-test")
            ...
            .build();
        profile.working_directory = Some(temp_dir.path().join("work").to_string_lossy().to_string());
        ...
        assert!(profile_workspace.join(".gemini/extensions").join(&ext1.id).exists());
        assert!(profile_workspace.join(".gemini/extensions").join(&ext2.id).exists());
