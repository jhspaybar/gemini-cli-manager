...
mod tests {
    ...
    use gemini_cli_manager::components::profile_form::{FormField, ProfileForm};
    ...
    fn test_extension_selection() {
        ...
        form.handle_events(Some(create_key_event(KeyCode::Char(' '))))
            .unwrap();
        assert_eq!(form.selected_extensions().len(), 1);

        // Move down and toggle second extension
        form.handle_events(Some(create_key_event(KeyCode::Down)))
            .unwrap();
        form.handle_events(Some(create_key_event(KeyCode::Char(' '))))
            .unwrap();
        ...
        form.handle_events(Some(create_key_event(KeyCode::Up)))
            .unwrap();
        form.handle_events(Some(create_key_event(KeyCode::Char(' '))))
            .unwrap();
        assert_eq!(form.selected_extensions().len(), 1);
    ...
    fn test_environment_variables() {
        ...
        form.handle_events(Some(gemini_cli_manager::tui::Event::Key(key_event)))
            .unwrap();
    ...
    fn test_form_with_all_fields() {
        ...
        assert_eq!(
            form.description_input().value(),
            "A complete profile with all fields"
        );
