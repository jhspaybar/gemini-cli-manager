...
mod tests {
    ...
    fn test_keyboard_navigation() {
        ...
        list.handle_events(Some(create_key_event(KeyCode::Home)))
            .unwrap();
        assert_eq!(list.selected_index(), 0);

        // Jump to bottom with End
        list.handle_events(Some(create_key_event(KeyCode::End)))
            .unwrap();
        assert_eq!(list.selected_index(), 2);
    }
    ...
    fn test_vim_navigation() {
        ...
        // j moves down
        list.handle_events(Some(create_key_event(KeyCode::Char('j'))))
            .unwrap();
        assert_eq!(list.selected_index(), 1);

        // k moves up
        list.handle_events(Some(create_key_event(KeyCode::Char('k'))))
            .unwrap();
        assert_eq!(list.selected_index(), 0);
    }
    ...
    fn test_search_mode() {
        ...
        // Type search query
        list.handle_events(Some(create_key_event(KeyCode::Char('t'))))
            .unwrap();
        list.handle_events(Some(create_key_event(KeyCode::Char('w'))))
            .unwrap();
        list.handle_events(Some(create_key_event(KeyCode::Char('o'))))
            .unwrap();
