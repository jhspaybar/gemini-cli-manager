...
mod tests {
    ...
    fn test_vim_navigation_keys() {
        ...
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Char('j'))))
            .unwrap();
        assert_eq!(result, Some(Action::Render));

        // Test vim key 'k' for up
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Char('k'))))
            .unwrap();
        assert_eq!(result, Some(Action::Render));
    }
    ...
    fn test_scrolling_functionality() {
        ...
        // Test scroll up
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Up)))
            .unwrap();
        assert_eq!(result, Some(Action::Render));
    }
    ...
    fn test_quit_key() {
        ...
        // Press 'q' to quit
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Char('q'))))
            .unwrap();
    ...
    fn test_actions_with_no_profile() {
        ...
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Char('l'))))
            .unwrap();
        assert!(result.is_none());
    }
    ...
    fn test_set_default_action() {
        ...
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Char('x'))))
            .unwrap();
        assert_eq!(result, None);
    }
    ...
    fn test_unknown_key_events() {
        ...
        let result = detail
            .handle_events(Some(create_key_event(KeyCode::F(1))))
            .unwrap();
        assert!(result.is_none());

        let result = detail
            .handle_events(Some(create_key_event(KeyCode::Char('z'))))
            .unwrap();
        assert!(result.is_none());
    }
    ...
    fn test_non_key_events() {
        ...
        let result = detail
            .handle_events(Some(gemini_cli_manager::tui::Event::Tick))
            .unwrap();
        assert!(result.is_none());
    }
    ...
    fn test_profile_with_sensitive_environment_variables() {
        ...
        profile.environment_variables.insert(
            "API_TOKEN".to_string(),
            "secret-token-value-here".to_string(),
        );
        profile
            .environment_variables
            .insert("SECRET_KEY".to_string(), "very-secret-key".to_string());
        profile
            .environment_variables
            .insert("PASSWORD".to_string(), "pwd".to_string()); // Short value
        profile
            .environment_variables
            .insert("NORMAL_VAR".to_string(), "normal-value".to_string());
