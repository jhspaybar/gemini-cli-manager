...
mod tests {
    ...
    use crate::test_utils::{ProfileBuilder, McpFixtures};
    ...
    fn test_create_launch_script() {
        ...
        profile.environment_variables.insert("PATH_VAR".to_string(), "/custom/path".to_string());
        ...
        assert!(content.contains(&format!("Launching Gemini CLI with profile: {}", profile.name)));
    ...
    fn test_install_extension_no_context() {
        ...
        let workspace = temp_dir.path().join(&profile.id);
        launcher.install_extensions_for_profile(&profile, &workspace).unwrap();
