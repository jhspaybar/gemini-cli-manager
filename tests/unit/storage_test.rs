#[cfg(test)]
mod tests {
    use crate::test_utils::{ExtensionBuilder, McpFixtures, ProfileBuilder, create_temp_storage};
    use chrono::Utc;
    ...
    fn test_extension_crud_operations() {
        ...
        storage
            .save_extension(&ext)
            .expect("Failed to save extension");

        // Read
        let loaded = storage
            .load_extension(&ext.id)
        ...
        storage
            .save_extension(&updated)
            .expect("Failed to update extension");

        let reloaded = storage
            .load_extension(&ext.id)
            .expect("Failed to reload extension");
        ...
        // Delete
        storage
            .delete_extension(&ext.id)
            .expect("Failed to delete extension");
    ...
    fn test_extension_context_persistence() {
        ...
        assert!(
            loaded
                .context_content
                .unwrap()
                .contains("Advanced Full-Featured Extension")
        );
    }
    ...
    fn test_profile_environment_variables() {
        ...
        assert_eq!(
            loaded.environment_variables.get("API_KEY"),
            Some(&"secret123".to_string())
        );
        assert_eq!(
            loaded.working_directory,
            Some("~/projects/test".to_string())
        );
    }
