...
mod tests {
    ...
    fn test_default_register_action_handler() {
        ...
        assert!(result.is_ok(), "Default register_action_handler should succeed");
    }
    ...
    fn test_default_register_config_handler() {
        ...
        assert!(result.is_ok(), "Default register_config_handler should succeed");
    }

    #[test]
    fn test_default_init() {
        let mut component = MockComponent::new();
        let area = Size { width: 80, height: 24 };
    ...
    fn test_default_handle_events() {
        ...
        assert!(result.is_ok(), "Default handle_events with None should succeed");
        assert!(result.unwrap().is_none(), "Should return None for None event");
        
        // Test with Tick event
        let result = component.handle_events(Some(Event::Tick));
        assert!(result.is_ok(), "Default handle_events with Tick should succeed");
        assert!(result.unwrap().is_none(), "Should return None for Tick event");
        
        // Test with Resize event
        let result = component.handle_events(Some(Event::Resize(100, 50)));
        assert!(result.is_ok(), "Default handle_events with Resize should succeed");
        assert!(result.unwrap().is_none(), "Should return None for Resize event");
    }
    ...
    fn test_handle_events_routes_to_key_handler() {
        ...
        assert!(result.is_ok(), "Should handle key event through handle_events");
    }
    ...
    fn test_handle_events_routes_to_mouse_handler() {
        ...
        assert!(result.is_ok(), "Should handle mouse event through handle_events");
    }
