...
mod tests {
    use crate::test_utils::{
        ExtensionBuilder, McpFixtures, ProfileBuilder, WorkspaceVerifier, create_temp_storage,
    };
    ...
    fn test_complete_user_journey_echo_server() {
        ...
        assert!(
            WorkspaceVerifier::verify_extension_installed(&profile_workspace, &echo_ext.id).is_ok()
        );
        ...
        assert!(
            config_json["mcpServers"]["echo"]["command"]
                .as_str()
                .unwrap()
                == "node"
        );
        ...
        // Check context file
        let context_content =
            WorkspaceVerifier::verify_context_file(&profile_workspace, &echo_ext.id, "GEMINI.md")
                .unwrap();
    ...
    fn test_multi_extension_profile_journey() {
        ...
        profile
            .environment_variables
            .insert("API_URL".to_string(), "http://localhost:3000".to_string());
        ...
        assert!(
            WorkspaceVerifier::verify_context_file(&profile_workspace, &multi_ext.id, "GEMINI.md")
                .is_ok()
        );

        assert!(
            WorkspaceVerifier::verify_context_file(
                &profile_workspace,
                &context_ext.id,
                "GEMINI.md"
            )
        ...
        assert_eq!(
            env.get("GEMINI_PROFILE"),
            Some(&"full-stack-dev".to_string())
        );
    }
    ...
    fn test_profile_update_journey() {
        ...
        assert!(
            WorkspaceVerifier::verify_extension_installed(&profile_workspace, &ext1.id).is_ok()
        );
        ...
        profile
            .environment_variables
            .insert("NEW_VAR".to_string(), "new_value".to_string());
        storage.save_profile(&profile).unwrap();
        ...
        assert!(
            WorkspaceVerifier::verify_extension_installed(&profile_workspace, &ext2.id).is_ok()
        );
    ...
    fn test_error_recovery_journey() {
        ...
        launcher
            .install_extensions_for_profile(&updated_profile, &profile_workspace)
            .unwrap();

        // Valid extension should still be installed
        assert!(
            WorkspaceVerifier::verify_extension_installed(&profile_workspace, &valid_ext.id)
                .is_ok()
        );
    ...
    fn test_context_file_variations_journey() {
        ...
        assert!(
            WorkspaceVerifier::verify_context_file(&profile_workspace, &ext1.id, "GEMINI.md")
                .is_ok()
        );

        assert!(
            WorkspaceVerifier::verify_context_file(&profile_workspace, &ext2.id, "GEMINI.md")
                .is_ok()
        );

        assert!(
            WorkspaceVerifier::verify_context_file(&profile_workspace, &ext3.id, "GEMINI.md")
                .is_ok()
        );

        // Extension 4 should not have a context file
        let ext4_context =
            WorkspaceVerifier::verify_context_file(&profile_workspace, &ext4.id, "GEMINI.md");
