...
impl McpFixtures {
    ...
    /// Create a URL-based server configuration
    pub fn url_based_server() -> McpServerConfig {
        McpServerConfig {
            url: Some("http://localhost:8080/mcp/v1".to_string()),
            command: None,
            args: None,
            cwd: None,
            env: Some({
    ...
    pub fn multi_server_extension() -> Extension {
        Extension {
            ...
            mcp_servers: {
                ...
                servers.insert("api-server".to_string(), Self::url_based_server());
                servers
            },
...
### api-server
URL-based server for API interactions
...
pub fn validate_extension_json(extension: &Extension) -> Result<(), String> {
    ...
    for (name, server) in &extension.mcp_servers {
        // Must have either command or URL, not both
        match (&server.command, &server.url) {
            (Some(_), Some(_)) => {
                return Err(format!("Server '{}' cannot have both command and URL", name));
            }
            (None, None) => {
                return Err(format!("Server '{}' must have either command or URL", name));
            }
            _ => {}
        }
...
mod tests {
    ...
    fn test_extension_json_format() {
        let ext = McpFixtures::echo_extension();
        let json = create_extension_json(&ext);
