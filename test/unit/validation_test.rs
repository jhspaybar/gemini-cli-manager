#[cfg(test)]
mod tests {
    use gemini_cli_manager::models::{Extension, Profile, extension::McpServerConfig};
    ...
    fn test_mcp_server_validation() {
        ...
        assert!(result.unwrap_err().contains("cannot have both command and URL"));
        ...
        let result = validate_extension_json(&ext4);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("must have either command or URL"));
    }
...
mod error_recovery_tests {
    ...
    fn test_corrupted_storage_recovery() {
        ...
        let ext = ExtensionBuilder::new("test").build();
